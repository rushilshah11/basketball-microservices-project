...
export default function TeamPage() {
    ...
    // Helper to process promises in batches to prevent "Proxy Failed" / Timeouts
    ...
    const loadTeamData = async () => {
        try {
            ...
            // Step 2: Fetch player details (Batch Search is fast)
            const playerDetails = await getPlayersBatch(playerNames);

            // Initialize state with players (loading predictions)
            const initialPlayers = playerDetails.map(p => ({ ...p, loading: true }));
        ...
        <div className="bg-white/10 backdrop-blur-sm rounded-xl p-6 border-2 border-white/20 hover:border-orange-500/50 transition-all duration-300 shadow-lg hover:shadow-xl">
            <div className="mb-4">
                ...
                <div className="flex items-center space-x-4 text-gray-300 text-sm">
                    ...
                    <span>⛹️ {player.position || 'N/A'}</span>
                </div>
            </div>

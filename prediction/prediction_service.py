...
# Import SessionLocal to create fresh sessions for background tasks
from database import PlayerPrediction, SessionLocal
...
class PredictionService:
    ...
    async def generate_prediction_for_player(
        ...
        if collect_training_data:
            try:
                # FIX 1: Create a wrapper to use a FRESH database session
                async def background_data_collection(p_name):
                    # Create new session exclusively for this background task
                    background_db = SessionLocal()
                    try:
                        await DataCollector.collect_and_store_game_logs(p_name, background_db, limit=10)
                    except Exception as e:
                        logger.error(f"Background collection failed for {p_name}: {e}")
                    finally:
                        background_db.close()

                # Schedule the task
                asyncio.create_task(background_data_collection(player_name))
        ...
        try:
            # FIX 2: Run CPU-bound model inference in a thread to avoid blocking the event loop
            loop = asyncio.get_running_loop()
            predicted_pts, predicted_ast, predicted_reb, confidence = await loop.run_in_executor(
                None,
                lambda: self._predict_performance(player_stats, home_game=True)
            )
        except Exception as e:
